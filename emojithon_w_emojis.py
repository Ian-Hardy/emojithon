...
class Parser:
		...
		if self.current_tok.type == TT_LPAREN:
			...
			if self.current_tok.type == TT_RPAREN:
				...
				if res.error:
					return res.failure(InvalidSyntaxError(
						self.current_tok.pos_start, self.current_tok.pos_end,
						"Expected ')', 'if', '⏱️', '⏳', '💪', int, float, identifier, '+', '-', '(' or '🚫'"
					))
	...
	def atom(self):
		...
		if tok.matches(TT_KEYWORD, '👌') or tok.matches(TT_KEYWORD, '🙅'):
			res.register_advancement()
			self.advance()
		...
		return res.failure(InvalidSyntaxError(
			...
			"Expected if, ⏱️, while, 💪, skip, int, float, identifier, '+', '-', '('"
		))
...
class Interpreter:
	...
	def visit_TF_Node(self, node, context):
		res = RTResult()
		if node.tok.matches(TT_KEYWORD, '👌'):
			result, error = Number(1).set_context(context), None
		elif node.tok.matches(TT_KEYWORD, '🙅'):
